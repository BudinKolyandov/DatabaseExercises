namespace Orm
{
    ...
    public abstract class DbContext
    {
        ...
        public void SaveChanges()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                var invalidEntities = dbSet
                    .Where(entity => !IsObjectValic(entity))
                    .ToArray();

                if (invalidEntities.Any())
                {
                    throw new InvalidOperationException(
                        $"{invalidEntities.Length} Invalid Entities found " +
                        $"in {dbSet.GetType().Name}!");                    
                }
            ...
            where TEntity : class, new()
        {
            ...
            {
                this.connection.UpdateEntities(modifiedEntities, 
                    tableName, columns);
            }
            if (dbSet.ChangeTracker.Deleted.Any())
            {
                this.connection.DeleteEntities(dbSet.ChangeTracker.Deleted,
                    tableName, columns);
            }
        ...
        private static bool IsObjectValic(object e)
        {
            var validationContext = new ValidationContext(e);
            var validationErrors = new List<ValidationResult>();

            var validationResult =
                Validator.TryValidateObject(e, validationContext, 
            ...
            where TdbSet : class, new() where TCollection : class, new()
        {
            ...
            foreach (var entity in dbSet)
            {
                ...
                var navigationEntities = navigationDbSet
                    .Where(navigatioEntity => primaryKey.GetValue(navigatioEntity)
            ...
            where TEntity : class
        {
            ...
            var columnns = GetEntityColumnNames(table);
            var tableName = GetTableName(table);
            var fetchedRows = this.connection.FetchResultSet<TEntity>(tableName, columnns)
                .ToArray();
            return fetchedRows;
        }
