namespace Orm    
{
    ...
    internal class DatabaseConnection
    {
        ...
        private SqlCommand CreateCommand(string query, params SqlParameter[] parameters)
        {
            var command = new SqlCommand(query, this.connection, this.transaction);
        ...
        public IEnumerable<string> GetCollumnNames(string tableName)
        {
            var rows = new List<string>();
            var queryText = $@"SELECT COLUMN_NAME FROM INFORMATIONSCHEMA.COLUMNS "
        ...
        public IEnumerable<T> FetchResultSet<T>(string tableName, params string[] columnNames)
        {
            ...
            var escapedColumn = string.Join(", ", columnNames.Select(EscapeColumn));
            var queryText = $@"SELECT {escapedColumn} FROM {tableName}";
            using (var query = CreateCommand(queryText))
            {
                using (var reader = query.ExecuteReader())
                {
            ...
            where T : class
        {
            ...
            var sqlRows = string.Join(", ",
                rowParameterNames.Select(x =>
                string.Format("({0})",
                string.Join(", ", x.Select(a=>$"@{a}")))));
            ...
            where T : class
        {
            ...
            foreach (var entity in modifiedEntities)
            {
                ...
                var primaryKeySql = string.Join(" AND ",
